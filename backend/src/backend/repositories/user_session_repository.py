...
class UserSessionRepository:
    ...
    async def get_session_by_token(session_token: str) -> Optional[UserSession]:
        return await UserSession.get_or_none(
            session_token=session_token,
            is_active=True,
        ).prefetch_related("user")

    @staticmethod
    async def validate_session(session_token: str) -> Optional[UserSession]:
        session = await UserSessionRepository.get_session_by_token(session_token)
