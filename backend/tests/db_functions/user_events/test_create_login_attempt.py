...
@pytest.mark.asyncio
async def test_create_login_attempt_success(mock_user_event, mock_event_schema) -> None:
    # Arrange
    ...
    with (
        ...
        mock.patch(
            "backend.db_functions.user_events.create_login_attempt.user_event_to_schema",
            new=mock.AsyncMock(return_value=mock_event_schema),
        ) as mock_converter,
    ):
        # Act
        result = await create_login_attempt(
        ...
        mock_create.assert_called_once_with(
            ...
            ip_address=test_ip_address,
            user_agent=test_user_agent,
            metadata={"success": test_success},
        )
        mock_converter.assert_called_once_with(mock_user_event)


@pytest.mark.asyncio
async def test_create_login_attempt_failure() -> None:
    # Arrange
    ...
    mock_event.ip_address = test_ip_address
    mock_event.user_agent = test_user_agent
    mock_event.metadata = {"success": False}
    ...
    mock_schema = UserEventSchema(
        ...
        ip_address=mock_event.ip_address,
        user_agent=mock_event.user_agent,
...
@pytest.mark.asyncio
async def test_create_login_attempt_without_user_id() -> None:
    # Arrange
    test_ip_address = "192.168.1.1"
    test_user_agent = "Mozilla/5.0"
    test_success = False
...
@pytest.mark.asyncio
async def test_create_login_attempt_database_error() -> None:
    # Arrange
    test_user_id = uuid.uuid4()
    test_ip_address = "192.168.1.1"
    test_user_agent = "Mozilla/5.0"
...
@pytest.mark.asyncio
async def test_create_login_attempt_converter_error(mock_user_event) -> None:
    # Arrange
    test_user_id = mock_user_event.user_id
    test_ip_address = mock_user_event.ip_address
    test_user_agent = mock_user_event.user_agent
