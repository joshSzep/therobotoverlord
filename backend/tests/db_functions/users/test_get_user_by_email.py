...
@pytest.mark.asyncio
async def test_get_user_by_email_success(mock_user, mock_user_schema) -> None:
    # Arrange
    test_email = mock_user.email
    ...
    with (
        ...
        mock.patch(
            "backend.db_functions.users.get_user_by_email.user_to_schema",
        ...
        mock_get.assert_called_once_with(email=test_email)
...
@pytest.mark.asyncio
async def test_get_user_by_email_database_error() -> None:
    # Arrange
    ...
    with mock.patch.object(
        ...
        with pytest.raises(IntegrityError) as exc_info:
            await get_user_by_email(test_email)
