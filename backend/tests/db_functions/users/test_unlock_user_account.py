...
def mock_user_schema(mock_user) -> UserSchema:
    return UserSchema(
        ...
        role=mock_user.role,
        is_locked=False,  # Should be updated after unlocking
...
@pytest.mark.asyncio
async def test_unlock_user_account_success(mock_user, mock_user_schema) -> None:
    # Arrange
    ...
    with (
        ...
        mock.patch(
            "backend.db_functions.users.unlock_user_account.user_to_schema",
            new=mock.AsyncMock(return_value=mock_user_schema),
        ) as mock_converter,
    ):
        # Act
        result = await unlock_user_account(test_id)
...
@pytest.mark.asyncio
async def test_unlock_user_account_user_not_found() -> None:
    # Arrange
    test_id = uuid.uuid4()
...
@pytest.mark.asyncio
async def test_unlock_user_account_already_unlocked(
    mock_user, mock_user_schema
) -> None:
    # Arrange
    test_id = mock_user.id
    mock_user.is_locked = False  # User is already unlocked
    mock_user.failed_login_attempts = 2  # Should still be reset to 0
    ...
    with (
        ...
        # Verify function calls - should still reset failed attempts and save
        mock_get.assert_called_once_with(id=test_id)
        assert mock_user.is_locked is False
        assert mock_user.failed_login_attempts == 0
        mock_user.save.assert_called_once()
...
@pytest.mark.asyncio
async def test_unlock_user_account_database_error() -> None:
    # Arrange
    test_id = uuid.uuid4()
    ...
    with mock.patch.object(
        ...
        with pytest.raises(OperationalError) as exc_info:
            await unlock_user_account(test_id)
...
@pytest.mark.asyncio
async def test_unlock_user_account_save_error(mock_user) -> None:
    # Arrange
    test_id = mock_user.id
...
@pytest.mark.asyncio
async def test_unlock_user_account_converter_error(mock_user) -> None:
    # Arrange
    test_id = mock_user.id
